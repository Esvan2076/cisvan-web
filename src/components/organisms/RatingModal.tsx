...
const RatingModal: React.FC<RatingModalProps> = ({ open, onClose, tconst }) => {
  ...
  const { reviewData, loading, error } = useReviewData(tconst, open);
  ...
  useEffect(() => {
    if (reviewData) {
      ...
      setRatings((prev) => {
        if (prev.hasOwnProperty(reviewData.tconst)) {
          return prev;
        }
        return {
          ...prev,
  ...
  useEffect(() => {
    const updateStarSize = () => {
      if (window.innerWidth < 640) {
        setStarSize("text-3xl");
      } else if (window.innerWidth < 1024) {
        setStarSize("text-4xl");
      } else {
        setStarSize("text-5xl");
      }
    };
    updateStarSize();
  ...
  const handleSubmit = () => {
    ...
    // Verificar que el título de la película tenga una calificación
    const movieRating = ratings[reviewData?.tconst || ""];
    if (movieRating === undefined || movieRating === 0) {
      toast.error(t("rating_required"));
      return;
    }

    const filteredGenres = reviewData?.genres
    ...
    const filteredActors = reviewData?.actors
    ...
    const filteredDirectors = reviewData?.directors
    ...
    const reviewJson = {
      tconst: selectedId,
      score: movieRating, // Usar el valor almacenado en el objeto ratings
      commentText: text,
    ...
    console.log("Reseña enviada:", JSON.stringify(reviewJson, null, 2));
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[1000] select-none">
        <div className="relative bg-neutral-800 p-6 rounded-lg w-[800px] max-w-[95vw] mx-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="text-white text-xl font-bold mb-2">
            {reviewData &&
              renderSelectableItem(reviewData.tconst, reviewData.primaryTitle)}
          </div>

          {/* Genres */}
          <div className="text-white font-semibold mb-1">{t("genres")}:</div>
          ...
          {/* Directors */}
          <div className="text-white font-semibold mb-1">{t("directors")}:</div>
          <div className="flex gap-2 flex-wrap mb-3">
          ...
          {/* Actors */}
          <div className="text-white font-semibold mb-1">{t("actors")}:</div>
