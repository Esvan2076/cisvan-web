      ...
      <main className="flex-1 text-white flex flex-col gap-24 px-2 sm:px-3 md:px-4 lg:px-4 xl:px-4">
        <div className="w-full max-w-7xl mx-auto flex flex-col pt-12">
          ...
          <section className="relative pb-10">
            <h2 className="text-3xl font-bold mb-2 select-none">{t("trending")}</h2>
            {loading ? (
              <div className="flex justify-center py-10 text-lg select-none">{t("loading")}</div>
            ) : (
          ...
          {userList.length > 0 && (
            <section className="relative pb-10">
              <h2 className="text-3xl font-bold mb-2 select-none">{t("my_list")}</h2>
              {loadingUserList ? (
                <div className="flex justify-center py-10 text-lg select-none">{t("loading")}</div>
              ) : (
          ...
          <section className="relative pb-10">
            <h2 className="text-3xl font-bold mb-2 select-none">{t("top_series")}</h2>
            {loading ? (
              <div className="flex justify-center py-10 text-lg select-none">{t("loading")}</div>
            ) : (
          ...
          <section className="relative pb-10">
            <h2 className="text-3xl font-bold mb-2 select-none">{t("top_movies")}</h2>
            {loading ? (
              <div className="flex justify-center py-10 text-lg select-none">{t("loading")}</div>
            ) : (
